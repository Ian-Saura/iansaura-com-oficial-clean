...
const Admin: React.FC<AdminProps> = ({ user }) => {
  ...
  const loadRetentionMetrics = async () => {
    ...
    try {
      ...
      const analyticsRes = await fetch(`/api/analytics-metrics.php?admin_password=IanSaura2024Admin!`);
      if (analyticsRes.ok) {
      ...
      const res = await fetch(`/api/retention-metrics.php?admin_password=IanSaura2024Admin!`);
      if (res.ok) {
        const data = await res.json();
        if (data.error) {
