...
function sendOnboardingEmail($user, $type, $testMode) {
    ...
    return mail($user['email'], $subject, $html, implode("\r\n", $headers));
}

function getOnboardingTemplate($type, $user) {
    $rawName = $user['first_name'] ?: explode(' ', $user['full_name'] ?? '')[0] ?: 'Crack';
    $name = function_exists('ensureUtf8') ? ensureUtf8($rawName) : $rawName;
    $xp = $user['xp'] ?? 0;
    ...
    $templates = [
        ...
        'day7' => [
            'subject' => "🎁 $name, última oportunidad: $30/mes",
